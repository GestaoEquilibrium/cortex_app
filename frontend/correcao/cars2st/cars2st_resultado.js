// ============================================================================
// CORTEX_APP — Resultado CARS2-ST (laudo) — padrão visual ICA/ASSQ/BDI
// ============================================================================
// URL: ?aplicacao_id=<uuid>
//
// CARS2 — Versão Padrão. Heteroavaliação, 15 itens. Cada item tem alternativas
// próprias (item.opcoes {texto,valor}); grava o índice escolhido.
//
// IMPORTANTE — escala dobrada: o banco soma o 'valor' como inteiro, então os
// meios-pontos do CARS2 entram dobrados (1->2 ... 4->8). Aqui o laudo DIVIDE
// POR 2 para mostrar o escore bruto REAL do CARS2 (15-60) e a nota real por item
// (1; 1,5; ... 4). Gravidade (estratificada por idade) e Escore-T vêm do
// arquivo ../cars2_norms.json (padrão SRS-2) — preenchido pelo profissional.
// ============================================================================
...
(function() {
    ...
    const state = {
        ...
        normSet: null  // bloco de cars2_norms.json para CARS2-ST
    };
    ...
    async function carregarTudo() {
        // Lookup de normas (gravidade + Escore-T)
        try {
            const resp = await fetch('../cars2_norms.json?v=1', { cache: 'no-cache' });
            if (resp.ok) {
                CARS2_NORMS = await resp.json();
                state.normSet = CARS2_NORMS?.instruments?.[SIGLA_ESPERADA] || null;
            }
        } catch (e) {
            console.warn('cars2_norms.json não carregado:', e);
        }
        ...
        if (!norma) throw new Error('Norma CARS2-ST não cadastrada');
        state.norma = norma;
    ...
    // Nota DOBRADA da opção (2..8). O laudo divide por 2 para a nota real.
    ...
    function calcularResultados(correcao) {
        ...
        const respItem = {};       // índice da opção escolhida
        const notaRealItem = {};   // nota REAL do CARS2 (1; 1,5; ... 4)
        let totalDobrado = 0;
        ...
        return { respItem, notaRealItem, totalReal, respondidos, idade, grupoIdade, classif, tscore };
    }

    // ── Gravidade estratificada por idade (de cars2_norms.json) ──────────────
    ...
    function classificarGravidade(totalReal, grupo) {
        ...
        for (const b of (grupo.bands || [])) {
            if (totalReal >= b.min && totalReal <= b.max) return b;
        }
        // acima do último teto -> última banda
        const bands = grupo.bands || [];
        return bands.length ? bands[bands.length - 1] : null;
    }
    // ── Escore-T (de cars2_norms.json, preenchido pelo profissional) ─────────
    function lookupTscore(totalReal, grupo) {
        const by = state.normSet?.tscore?.by_raw;
        if (!by || !Object.keys(by).length) return null;
        // tenta chave inteira ("30") ou com .5 ("29.5")
        const k = Number.isInteger(totalReal) ? String(totalReal) : String(totalReal);
        const row = by[k] != null ? by[k] : by[String(parseFloat(totalReal))];
        ...
        <div class="laudo">
            <div class="laudo-header">
                <div class="laudo-header-esq">
                    ...
                    <div class="laudo-header-textos">
                        ...
                        <h1 class="laudo-header-titulo">CARS2-ST</h1>
                        <div class="laudo-header-subtitulo">Childhood Autism Rating Scale, Second Edition — Versão Padrão<br>15 itens · escore 1 a 4 por item · total bruto ${RAW_MIN}–${RAW_MAX}</div>
                    </div>
                </div>
            ...
            <div class="laudo-rodape">
                <div class="laudo-rodape-esq">
                    ...
                    <div class="laudo-rodape-tipo">Correção automatizada — CARS2-ST</div>
                </div>
    ...
    function renderResultado() {
        const s = state.scores;
        const total = s.totalReal;
        const grupo = s.grupoIdade;
        const cl = s.classif;

        if (!grupo) {
            return `
                <div class="bai-nivel-expl" style="border-left-color:#94a3b8;">
                    <strong>Escore bruto total: ${fmtNum(total)} de ${RAW_MAX}.</strong>
                    Não foi possível determinar o grupo etário (idade ausente) — a classificação de gravidade
                    do CARS2-ST é estratificada por idade. Verifique a data de nascimento do paciente.
                </div>`;
        }

        const bands = grupo.bands || [];
        const cards = bands.map(b => {
            const ativo = cl && b.slug === cl.slug;
            const cor = COR_SLUG[b.slug] || '#64748b';
            return `
                <div class="cars-nivel-card ${ativo ? 'ativo' : ''}" ${ativo ? `style="border-color:${cor};"` : ''}>
        ...
        return `
            <div class="cars-niveis-grid">${cards}</div>
            ${tscoreLinha}
            <div class="bai-nivel-expl" style="border-left-color:${cor};">
                ...
                (faixa etária de referência: ${escapeHtml(grupo.label)}).
            </div>`;
    }

    // Distribuição dos 15 itens por nível (nota real arredondada ao ponto âncora)
    function renderIntensidade() {
        ...
        let respondidos = 0;
        for (const item of state.itens) {
            const v = state.scores.notaRealItem[item.numero];
            if (v == null) continue;
            respondidos++;
            const idxB = Math.min(3, Math.max(0, Math.round(v) - 1)); // 1->0 ... 4->3
            buckets[idxB].n++;
        }
        ...
        const linhas = buckets.map(b => {
            ...
            return `
                <div class="bai-int-row">
                    ...
                    <span class="bai-int-val">${b.n} ${b.n === 1 ? 'item' : 'itens'}</span>
                </div>`;
        }).join('');
        return `
            <div class="bai-intensidade">
                ${linhas}
                <div class="bai-int-nota">Dos ${respondidos} itens, esta é a distribuição por nível de atipicidade (aproximada ao ponto âncora mais próximo; os meios-pontos aparecem no detalhamento técnico).</div>
            </div>`;
    }
    ...
    function renderDetalhesItens() {
        ...
        const linhas = state.itens.map(item => {
            ...
            const nota = state.scores.notaRealItem[item.numero];
            const frase = (idx != null) ? (opcaoTexto(item, idx) || '—') : '—';
            const corResp = nota == null ? '#cbd5e1'
                : (nota >= 3.5 ? '#dc2626' : nota >= 3 ? '#ea580c' : nota >= 2 ? '#d97706' : '#16a34a');
            return `<tr>
                ...
                <td style="text-align:center;font-weight:700;color:${corResp};">${fmtNum(nota)}</td>
            </tr>`;
        }).join('');
        return `
            <div class="bai-tab-itens">
                <table>
                    <thead><tr>
                        ...
                        <th>Item · alternativa marcada</th>
                        <th style="text-align:center;width:64px;">Nota</th>
                    </tr></thead>
                    <tbody>${linhas}</tbody>
                </table>
                <div class="cars-tab-nota">Escore bruto total = soma das 15 notas = <strong>${fmtNum(state.scores.totalReal)}</strong> (de ${RAW_MAX}).</div>
            </div>`;
    }
    ...
    async function gerarPDF() {
        ...
        try {
            ...
            pdf.save(`CARS2-ST - ${nome}_${formatarDataArquivo(new Date())}.pdf`);
