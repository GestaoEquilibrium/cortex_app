...
(function() {
    ...
    function renderLaudo() {
        ...
        return `
        <div class="laudo">
            <div class="srs2-laudo-header">
                <div class="srs2-header-esq">
                    <div class="srs2-header-logo">E</div>
                    <div>
                        <div class="srs2-header-supratitulo">Relatório Neuropsicológico</div>
                        <h1 class="srs2-header-titulo">SRS-2</h1>
                        <div class="srs2-header-subtitulo">Escala de Responsividade Social — 2ª Edição</div>
                        <div class="srs2-header-modalidade">${escapeHtml(formLabel)}</div>
                    </div>
                </div>
                <div class="srs2-header-pontuacao">
                    <div class="srs2-header-pontuacao-label">Escore T Total</div>
                    <div class="srs2-header-pontuacao-valor">${totalT}</div>
                    <div class="srs2-header-pontuacao-classif" style="background:${totalClassif.bg};color:${totalClassif.text};">${totalClassif.label}</div>
                </div>
            </div>

            <div class="srs2-secao-titulo"><span class="srs2-secao-tag">1</span> Identificação</div>
            <div class="srs2-identificacao">
                <div class="srs2-identif-item"><span class="srs2-identif-label">Paciente:</span> <span class="srs2-identif-valor">${escapeHtml(state.paciente.nome_completo)}</span></div>
                <div class="srs2-identif-item"><span class="srs2-identif-label">Data de Avaliação:</span> <span class="srs2-identif-valor">${formatarDataBR(dataApl)}</span></div>
                <div class="srs2-identif-item"><span class="srs2-identif-label">Avaliador:</span> <span class="srs2-identif-valor">${escapeHtml(avaliador)}</span></div>
                <div class="srs2-identif-item"><span class="srs2-identif-label">Formulário:</span> <span class="srs2-identif-valor">${escapeHtml(formLabel)}</span></div>
            </div>

            <div class="srs2-secao-titulo"><span class="srs2-secao-tag">2</span> Perfil de Escores T</div>
            <div class="srs2-perfil-wrap">
                <div class="srs2-perfil-canvas-container"><canvas id="srs2-perfil-chart"></canvas></div>
            </div>

            <div class="srs2-secao-titulo"><span class="srs2-secao-tag">3</span> Tabela de Resultados</div>
            ${renderTabelaResultados()}

            <div class="srs2-secao-titulo"><span class="srs2-secao-tag">4</span> Detalhamento por Escala</div>
            ${SRS2_NORMS.scale_order.map(renderEscalaCard).join('')}

            <div class="srs2-secao-titulo"><span class="srs2-secao-tag">5</span> Interpretação Clínica do Escore T</div>
            ${renderInterpretacaoClinica()}

            <div class="srs2-nota-tecnica">
                <strong>Nota técnica:</strong> A SRS-2 (Constantino & Gruber, 2012)
                é um instrumento dimensional de rastreio para Transtorno do Espectro
                Autista (TEA), composto por 65 itens em 5 subescalas
                (Percepção, Cognição, Comunicação e Motivação Sociais; Padrões
                Restritivos e Repetitivos) e 2 compostos (Comunicação e Interação;
                Total). O Escore T tem média 50 e desvio-padrão 10 na população
                normativa. Pontuações elevadas indicam maior gravidade dos
                comportamentos associados ao TEA. Este instrumento deve ser
                interpretado em conjunto com entrevista clínica, observação direta
                e outros dados da avaliação neuropsicológica.
            </div>
