...
window.CortexDocumentos = (function () {
    ...
    // ── Assinatura digital ──────────────────────────────────────────────────
    // A janela de posicionamento vive em shared/assinatura.js, usada também
    // pela aba Laudo.

    function podeAssinar() {
        return !!window.CortexAssinatura && window.CortexAssinatura.podeAssinar();
    }

    function abrirAssinatura(id) {
        const d = ctx.itens.find(x => x.id === id);
        if (!d || !window.CortexAssinatura) return;
        window.CortexAssinatura.abrir({
            tipo: 'documento',
            id: d.id,
            bucket: BUCKET,
            path: d.arquivo_path,
            titulo: d.titulo,
            onAssinado: () => carregar(ctx.pacienteId)
        });
    }
