...
// URL: /responder/etdahpais.html?token=<uuid>
//
// Heteroaplicação: pai/mãe/responsável responde sobre o(a) filho(a).
// Fluxo:
...
//
// Escala 1-6 (Nunca → Muito frequentemente). Janela: últimos 6 meses.
    ...
    function renderConsentimento() {
        return `
            <div class="tela-consentimento">
                ...
                <h1>Olá!</h1>
                <p class="subtitulo">Você foi convidado(a) a responder o questionário <strong>${escapeHtml(state.norma.versao_label)}</strong> sobre seu(sua) filho(a).</p>

                <h2>Antes de começar</h2>
                <p>Este questionário foi solicitado por um profissional como parte da avaliação do(a) seu(sua) filho(a). Você responde como <strong>pai, mãe ou responsável</strong>. As respostas serão analisadas exclusivamente pelo(a) profissional.</p>

                <h2>Como responder</h2>
                <ul>
                    <li>São <strong>${state.itens.length} afirmações</strong> sobre comportamentos que seu(sua) filho(a) pode apresentar. Leia cada uma com atenção.</li>
                    <li>Considere a <strong>frequência</strong> de cada comportamento <strong>no momento atual</strong>, desde que ocorra <strong>nos últimos seis meses</strong> (de <em>Nunca</em> a <em>Muito frequentemente</em>).</li>
    ...
    function renderPerguntas() {
        ...
        let html = `
            <div class="tela-perguntas-instrucoes">
                <p>Para cada afirmação, marque a <strong>frequência</strong> com que o comportamento ocorre com seu(sua) filho(a) nos <strong>últimos seis meses</strong> — de <em>Nunca</em> a <em>Muito frequentemente</em>.</p>
            </div>
