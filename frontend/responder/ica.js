// ============================================================================
// CORTEX_APP — YSQ-S3 Online (Questionário de Esquemas de Young — autoaplicado)
// ============================================================================
// Página pública SEM autenticação.
// URL: /responder/ysqs3.html?token=<uuid>
//
// Fluxo:
//   1. Tela 1: Termo de consentimento
//   2. Tela 2: 90 itens em lista única com scroll (Likert 1-6)
//   3. Tela 3: Agradecimento ("Resposta enviada. Obrigado.")
//
// Escala YSQ-S3 — Jeffrey Young / Adapt. BR: GAAPCC/PUCRS (2020)
//   90 itens. Escala 1-6 (Completamente falso ... Me descreve perfeitamente)
//   18 esquemas iniciais desadaptativos × 5 itens cada
//   Agrupados em 5 domínios.
//   Cálculo: média por esquema (soma 5 ÷ 5). Corte clínico: média ≥ 3,0.
//   Faixas: 1.0-2.0 ausente · 2.1-3.0 leve · 3.1-4.0 moderado ·
//           4.1-5.0 intenso · 5.1-6.0 muito intenso.
//
// Cálculo no banco quando paciente clica "Enviar":
//   1. Soma respostas brutas (90 itens linkados aos 18 esquemas)
//   2. Médias e classificações recalculadas no JS do laudo (decisão B)
...
(function() {
    ...
    const state = {
        ...
        norma: null,
        itens: [],
        respostas: {},  // {numero: 0|1}  (0=Não, 1=Sim)
    ...
    function renderConsentimento() {
        return `
            <div class="tela-consentimento">
                ...
                <h1>Olá!</h1>
                <p class="subtitulo">Você foi convidado(a) a responder a escala <strong>${escapeHtml(state.norma.versao_label)}</strong> sobre uma criança que você acompanha.</p>

                <h2>Antes de começar</h2>
                <p>Esta escala foi solicitada por um profissional como parte de uma avaliação. Você responde como <strong>informante</strong> (professor[a], pai, mãe ou responsável), com base no que <strong>observou diretamente</strong> no comportamento da criança.</p>

                <h2>Como responder</h2>
                <ul>
                    <li>São <strong>${state.itens.length} comportamentos</strong>. Para cada um, marque <strong>Sim</strong> se a criança <strong>apresenta</strong> o comportamento ou <strong>Não</strong> se <strong>não apresenta</strong>.</li>
                    <li>Baseie-se no comportamento <strong>habitual</strong> da criança, no que você observa no dia a dia.</li>
                    <li>Baseie-se apenas no que observou — não em suposições. Avalie cada item de forma independente.</li>
                ...
                <ul>
                    ...
                    <li>Apenas o profissional responsável terá acesso aos resultados.</li>
                    <li>Os dados serão usados apenas para apoiar a avaliação.</li>
                </ul>

                <label class="consentimento-aceite">
                    <input type="checkbox" id="check-consentimento">
                    <span class="consentimento-aceite-texto">
                        Li e concordo em prosseguir com o preenchimento.
                    </span>
                </label>
    ...
    function renderPerguntas() {
        ...
        let html = `
            <div class="tela-perguntas-instrucoes">
                <p>Para cada comportamento, marque <strong>Sim</strong> se a criança o <strong>apresenta</strong> ou <strong>Não</strong> se <strong>não apresenta</strong>, com base no que você observa no dia a dia. Avalie cada item de forma independente.</p>
            </div>
